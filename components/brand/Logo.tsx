...
export function Logo({ size = 48, className, priority = false }: { size?: number; className?: string; priority?: boolean }) {
	return (
		<span className={cn('relative inline-block shrink-0', className)} style={{ width: size, height: size }}>
			<Image src='/logo.png' alt='Top Dogs' fill priority={priority} sizes={`${size}px`} style={{ objectFit: 'contain' }} />
		</span>
	);
}
		...
		<span className={cn('inline-flex items-center gap-3', className)}>
			...
			{withText && (
				<span className='leading-none'>
					<span className='block font-[family-name:var(--font-display)] text-xl tracking-[0.18em] text-[var(--color-cream)]'>TOP DOGS</span>
